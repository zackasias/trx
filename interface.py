...
class ModuleInterface:
    ...
    def valid_account(self):
        if not self.disable_subscription_check:
            try:
                # Do the full account setup sequence
                account_info, subscription_data = self.session.setup_account()
                ...
                # Verify subscription bundle is LINK
                bundle = subscription_data.get('subscription', {}).get('bundle', {})
                if not bundle or bundle.get('plan_code') != 'bp_link':
                    raise self.exception('Beatport: Account does not have an active "Link" subscription')
                ...
                # Only raise exception if it's not a push notification error
                if "subscribing to topic" not in str(e):
                    logging.error(f"Error validating account: {str(e)}")
                    raise self.exception('Failed to validate Beatport account')
            except Exception as e:
                # Check if it's a push notification error
                if "subscribing to topic" not in str(e):
                    logging.error(f"Error validating account: {str(e)}")
                    raise self.exception('Failed to validate Beatport account')
    ...
    def get_track_download(self, track_id: str, quality_tier: QualityEnum) -> TrackDownloadInfo:
        ...
        try:
            ...
            # Pass auth token to stream instance
            self.stream.auth.access_token = self.session.access_token
            ...
            # Return download info with correct fields from models.py
